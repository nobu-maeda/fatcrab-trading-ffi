...
use fatcrab_trading::taker::{FatCrabTakerAccess, TakerBuy, TakerSell, FatCrabTakerNotif};
use tokio::sync::mpsc;
...
impl FatCrabBuyTaker {
    ...
    pub fn notify_peer(&self, txid: String) -> Result<(), FatCrabError> {
        RUNTIME.block_on(async { self.inner.notify_peer(txid).await }).map_err(|e| e.into())
    }
    ...
    pub fn register_notif_delegate(
        &self,
        delegate: Arc<dyn FatCrabTakerNotifDelegate>) -> Result<(), FatCrabError> {
            ...
            while let Some(notif) = rx.recv().await {
                match notif {
                    FatCrabTakerNotif::TradeRsp(trade_rsp_envelope) => {
                        delegate.on_taker_trade_rsp_notif(Arc::new(trade_rsp_envelope));
                    }
                    FatCrabTakerNotif::Peer(peer_envelope) => {
                        delegate.on_taker_peer_notif(Arc::new(peer_envelope));
                    }
                }
            }
        });
